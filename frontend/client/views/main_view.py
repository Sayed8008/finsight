...
from client.core.animation import VIEW_MS, slide_fade_in
...
class MainView(QWidget):
    ...
    def _show_section(self, key: str) -> None:
        ...
        # A fade with a few pixels of rise, so a section arrives rather than
        # blinking into place. After `setCurrentIndex`, so the widget animated
        # is the one now on screen — and non-blocking, so it stays clickable
        # throughout, including the controls that are still fading in.
        slide_fade_in(self.pages.currentWidget(), VIEW_MS)
