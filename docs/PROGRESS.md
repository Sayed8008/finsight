...
- Opening a section costs one request every time (ADR-041). Deliberate: the
  alternative is a screen showing what was true when it was first opened, which
  is what it used to do.
- The charts retain a few invisible graphics items per redraw (ADR-042). They
  paint nothing; the visible half of that defect is fixed and tested.
